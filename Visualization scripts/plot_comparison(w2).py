...
def plot_comparison(file1, file2, file3, file4, file5, file6, panel_labels=('a', 'b', 'c', 'd')):
    ...
    for f in files:
        ...
        label = f.replace("ReleaseProfile(w2)/AverageOverSimulations", "").replace(".txt", "")
        labels.append(label)
    ...
    ax.legend(loc='best')
    plt.tight_layout()
    plt.savefig(f'ReleaseProfile EntireLattice (all_w2).png')
    plt.close()
    ...
    ax.legend(loc='best')
    plt.tight_layout()
    plt.savefig(f'ReleaseProfile FriendlyLayer (all_w2).png')
    plt.close()
    ...
    ax.set(xlabel='$t_{MC}$', ylabel='Percentage of ants',  xscale = 'log', ylim=(0, 100))
    #title='For blind layer',
    ax.yaxis.set_major_locator(mticker.MultipleLocator(20))
    ax.yaxis.set_minor_locator(mticker.MultipleLocator(10))
    ax.legend(loc='best')
    plt.tight_layout()
    plt.savefig(f'ReleaseProfile BlindLayer (all_w2).png')
    plt.close()
    ...
    ax.legend(loc='best')
    plt.tight_layout()
    plt.savefig(f'Percentage Released (all_w2).png')
    plt.close()
...
def main():
    ...
    # Define six w2 values and build filenames accordingly
    w2_values = [0.0, 0.1,0.2,0.4,0.6,0.8]
    file1, file2, file3, file4, file5, file6 = [f"ReleaseProfile(w2)/AverageOverSimulations(w2={v}).txt" for v in w2_values]

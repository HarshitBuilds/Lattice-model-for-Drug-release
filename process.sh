...
do
for tau_val in {1..5}; #tau value for calculating mean square displacement
do
s="run_w1_${w1}_w2_${w2}_p_${p}_a_${a}_n_${n}_x_${x}_pj_${pj}"
mkdir "$s" -p 
cd "$s"
sweeps=100 #number of MC sweeps in simulation
mcruns=1 #number of MC runs to average over
for ((r=1;r<=mcruns;r++)); 
do
  gdb --args ../ANT --WALLF1 "$w1" --WALLF2 "$w2" -p "$p" -a "$a" -r "$r" -s "$sweeps" -S 1000000 -n "$n" -x "$x" --PJUMP "$pj" --tau_val "$tau_val"
done
